...
🌙 The same nineteen **[in the dark](SCREENSHOTS.md#in-the-dark)** — eight colour tokens swapped by
`prefers-color-scheme`, so the whole application follows the operating system with no switch to find.

📐 **[On a tablet](SCREENSHOTS.md#on-a-tablet)** — its own layout rather than a wide phone: the page columns stack
below 1024px, while the grids inside a card stay two across all the way down to 640px.

📱 **[On a phone](SCREENSHOTS.md#on-a-phone)** — one column throughout, filters stacked, and no screen scrolling
...
`?role=` filters the same way and is **AND'd with the states**: `?role=OWNER&status=PENDING` is the owners still
waiting, and finds nobody once that invitation is withdrawn even though `?role=OWNER` alone still finds it. Two
filters are always both; several values inside one are always either.

The role helpers live on `TenantRole` now — `matching(query)` for the search and `parseAll(names)` for the filter —
because the membership list and the invitation list both need them, and knowing what a role is called belongs with the
...
| `GET`  | `/api/organizations/{slug}/invitations` | **owner** | A **page** of its invitations; `?q=` searches, `?status=` and `?role=` filter |
...
what a search should reach. It keeps `sortOrder`, because that is the order a dropdown offers these in and paging by
id would show them in whatever order somebody inserted them.

**It now answers 404 when the tenant keeps no such list**, where it used to answer an empty one. That was there so a
dropdown would not break, and no dropdown reads this endpoint — they all read the whole map at `/reference-data`,
which is unchanged and still simply omits a category it has no rows for. The endpoint had to choose, because "there is
no such list" and "nothing matched what you typed" are the same empty page otherwise, and a screen that cannot tell
them apart cannot say anything useful.

The UI reads it at `/organizations/{slug}/reference-data/{category}`, which shows the label, the code a record stores,
the order, whether the value is switched off and whether it came with the tenant.
